...
void sortDescendingOrderBasedOnHighSalary(employee e[] , int distinctCount);
...
int main()
{
	...
	sortDescendingOrderBasedOnHighSalary(e , distinctCount); 
...
void sortDescendingOrderBasedOnHighSalary(employee e[] , int distinctCount)
{
	...
	/* sort the structure based on high salary*/
	for(i=0; i<distinctCount-1; i++)
	{
		swapped = 0;
		for(j=0; j<distinctCount-1; j++)
		{
			if(e[j].highSalary < e[j + 1].highSalary)
			{
				temp = e[j];
				e[j] = e[j + 1];
				e[j + 1] = temp;
